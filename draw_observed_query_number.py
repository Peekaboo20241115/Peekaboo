...
def draw_comparison(Observed_query_number_per_timeslot,is_fvp,dataset,test_times):
    ...
    for observed_query_number_per_timeslot in Observed_query_number_per_timeslot:
        with open("./results/query_number/Jigsaw_plus_"+dataset+"_is_fvp_"+str(int(is_fvp))+"_observed_query_number_per_timeslot_"+str(observed_query_number_per_timeslot)+".pkl","rb") as f:
        ...
        with open("./results/query_number/SAP_plus_"+dataset+"_is_fvp_"+str(int(is_fvp))+"_observed_query_number_per_timeslot_"+str(observed_query_number_per_timeslot)+".pkl","rb") as f:
            Acc_Sap_plus = pickle.load(f)
            ACC_Sap_plus.append(Acc_Sap_plus)
            ACC_Sap_plus_average.append(np.average(Acc_Sap_plus))

        with open("./results/query_number/FMA_"+dataset+"_is_fvp_"+str(int(is_fvp))+"_observed_query_number_per_timeslot_"+str(observed_query_number_per_timeslot)+".pkl","rb") as f:
            Acc_FMA = pickle.load(f)
            ACC_FMA.append(Acc_FMA)
            ACC_FMA_average.append(np.average(Acc_FMA))

        with open("./results/query_number/SPJigsaw_plus_"+dataset+"_is_fvp_"+str(int(is_fvp))+"_observed_query_number_per_timeslot_"+str(observed_query_number_per_timeslot)+".pkl","rb") as f:
        ...
        with open("./results/query_number/SPSAP_plus_"+dataset+"_is_fvp_"+str(int(is_fvp))+"_observed_query_number_per_timeslot_"+str(observed_query_number_per_timeslot)+".pkl","rb") as f:
    ...
    plt.ylim((0,1.01))
    plt.xlabel("Obsesrved Query Number")
    plt.ylabel("Accuracy")
    plt.xticks([1,2,3,4],[1000,2500,5000,10000])
    if dataset=="Enron" and is_fvp == False:
        legend = plt.legend([Bp_Jigsaw_plus["boxes"][0], Bp_sp_Jigsaw_plus["boxes"][0],Bp_Sap_plus["boxes"][0], Bp_sp_Sap_plus["boxes"][0],Bp_FMA["boxes"][0]],\
                ["Jigsaw+","Jigsaw+ with SP","Sap+","Sap+ with SP","FMA"],\
                loc=4, bbox_to_anchor=(1,0.18),\
    ...
    plt.savefig("./results/query_number/test_query_number_"+dataset+"_isfvp_"+str(int(is_fvp))+".pdf",bbox_inches='tight')
