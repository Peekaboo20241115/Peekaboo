...
def draw(dataset):
    with open("results/peekaboo_ari_test_1_"+dataset+"_isfvp_"+str(int(True))+".pkl", "rb") as f:
        ARI_fvp,RI_fvp = pickle.load(f)
    with open("results/peekaboo_ari_test_1_"+dataset+"_isfvp_"+str(int(False))+".pkl", "rb") as f:
    ...
    Positions_AP = np.array([1,2,3,4])-0.175
    ...
    Positions_FVP = np.array([1,2,3,4])+0.175
    ...
    plt.ylabel("ARI")
    plt.xlabel("Observation Days in a Round")
  
    plt.legend([Bp_AP["boxes"][0], Bp_FVP["boxes"][0]],\
            ["AP","FVP"],\
            #loc="center left", bbox_to_anchor=(-0.015, 0.5),\
            fancybox=True, shadow=True)
    plt.savefig("./results/test_peekaboo_1_"+dataset+".pdf",bbox_inches='tight')
